...
func Level3(lever, turnedOn):
	...
	match(name):
		"1":
			lever.visible = false
		#Lever 1 of Door Puzzle [Lever Down y = 183, lever up y = -183]
		"2":
			var door1 = get_parent().get_node("Door1")
			var door2 = get_parent().get_node("Door2")
			var door4 = get_parent().get_node("Door4")
			if(turnedOn):
				door1.position.y = -183
				door2.position.y = -183
				door4.position.y = 183			
			elif(!turnedOn):
				door1.position.y = 183
				door2.position.y = 183
			pass
		...
		"4":
			var door1 = get_parent().get_node("Door1")
			var door3 = get_parent().get_node("Door3")
			if(turnedOn):
				door1.position.y = -183
				door3.position.y = 183			
			elif(!turnedOn):
				door1.position.y = 183
			pass
